...
#ifndef __PFS_TESTENV_H__
#define __PFS_TESTENV_H__
...
#define CHECK_CUR_OFFSET(fd_, val) do {          \
    off_t pos = pfs_lseek(fd_, 0, SEEK_CUR);    \
    EXPECT_EQ(pos, val);                \
} while(0)
...
#define CHECK_FILETYPE(pbdpath, type) do {        \
    struct stat fstat;          \
    pfs_stat(pbdpath , &(fstat));       \
    EXPECT_TRUE(S_IS##type(fstat.st_mode));    \
} while(0)


class PFSTestEnv : public testing::Environment
{
public:
    explicit PFSTestEnv(const std::string &cluster, const std::string &pbdname, int hostid) :
        cluster_(cluster),
        pbdname_(pbdname),
        hostid_(hostid) {
        }
